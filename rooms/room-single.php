	...
	if(isset($_GET['id'])) {
		...
		if (isset($_POST['submit'])) {
			...
			OR empty($_POST['check_in']) OR empty($_POST['check_out'])) {
				...
				$today = new DateTime('today'); // get today's date at 00:00:00
				if ($check_in < $today || $check_out <= $today) {
				echo "<script>alert('Please pick a check-in and check-out date that are not in the past. Check-in must be at least today or later.');</script>";
